...
The Dice coefficient for lesions is calculated as an average across all lesions. For a detailed list of metrics, please refer to the [LiTS_metrics_summary.csv](evaluation%2FLiTS_metrics_summary.csv). 
...
- General Training Command on GPU 0. **All models are trained for 100 epochs**. Please note that all model variants, with the exception of nnUNet, are trained without deep supervision.
   ```bash
