...
class AddGuidanceJSON(Randomizable, MapTransform):
    ...
    def __call__(self, data: Mapping[Hashable, torch.Tensor]) -> Mapping[Hashable, torch.Tensor]:
        ...
        for _, (key_label, _) in enumerate(data[LABELS_KEY].items()):
            if '.json' in self.json_dir:
                json_fn = self.json_dir 
            else:
                im_fn = data['label_meta_dict']['filename_or_obj'][0].split('/')[-1] if len(data['label_meta_dict']['filename_or_obj']) == 1 else data['label_meta_dict']['filename_or_obj'].split('/')[-1]
