...
def get_network(network_str: str, labels: Iterable, non_interactive: bool = False, anat_pred: bool = False):
    """
    ...
    in_channels = in_channels + 1 if anat_pred else in_channels
...
def create_supervised_evaluator(args, resume_from="None") -> SupervisedEvaluator:
    ...
    network = get_network(args.network, args.labels, args.non_interactive, args.anat_pred).to(device)
...
def get_trainer(
    ...
    network = get_network(args.network, args.labels, args.non_interactive, args.anat_pred).to(sw_device)
